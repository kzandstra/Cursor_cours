...
function App() {
  ...
  const handleUploadDevis = async () => {
    ...
    try {
      ...
      // On utilise le proxy local configuré dans vite.config.ts pour éviter l'erreur CORS
      // On retire seulement le domaine pour appeler le relatif
      let webhookUrl = import.meta.env.VITE_N8N_WEBHOOK_URL || '/webhook/upload-devis-ia';
      if (webhookUrl.startsWith('http')) {
        const urlObj = new URL(webhookUrl);
        webhookUrl = urlObj.pathname; // ex: '/webhook/upload-devis-ia'
      }
            ...
            <div className="modal-body">
              ...
              <div className="modal-actions">
                ...
                <button className="primary-button" onClick={handleUploadDevis} disabled={uploadStatus === 'uploading'}>
                  {uploadStatus === 'uploading' ? 'Envoi...' : 'Envoyer votre devis'}
                </button>
              </div>
              <div className="hint">
